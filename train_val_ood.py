...
def train_model(model, data_loaders, optimizer, num_epochs, scheduler=None):
    ...
    if args.noise != 0.0:
        ...
        matplotlib.use("Agg")
        # 将数据分为已知类和未知类
        known_ig = best_ig[label != -1]
        unknown_ig = best_ig[label == 5]
        ...
        sns.kdeplot(data=data, x='Ignorance', hue='Category', fill=True, common_norm=False, alpha=0.5, linewidth=1.5)
        plt.xlabel('Ignorance', fontsize=16)
        plt.ylabel('Density', fontsize=16)
        plt.title('Ignorance Distribution for Known and Unknown Classes', fontsize=16)
        plt.legend(title='Category', labels=['Known Classes', 'Unknown Classes'], fontsize=16, title_fontsize=16)
        plt.grid(True)
        plt.show()
...
def get_data_info(data_name):
    if data_name == 'HMDB':
        ...
        dims = None
        num_classes = None
        print("Choose the other data and input the view_dims and classes")
...
def run(args):
    ...
    data_set = MultiViewDataWithoutLeak(data_name, args.multiview, std=args.noise)
...
def parse_arguments():
    ...
    parser.add_argument("--data_name", default='CUB', type=str, help="Desired Data Name.")
    parser.add_argument("--batch_size", default=64, type=int, help="Desired batch size.")
